...
def load(request):   
    ...
    try:
        df = ds.dataset("delta/log/scada/log.parquet",filesystem=s3).to_table().to_pandas()
    except:
    ...
    if len(files_to_upload) != 0 :
      ...
      write_deltalake(delta_path, xx,mode='append',partition_by=['Date'],storage_options=storage_options)
      pq.write_table(log_tb,"delta/log/scada/log.parquet",filesystem=s3)
      DeltaTable(delta_path,storage_options=storage_options).create_checkpoint()
      return "done"
